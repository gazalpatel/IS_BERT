...
class LanguageTransformer(nn.Sequential):
    """
    Loads or create a LanguageTransformer model, that can be used to map sentences / text to embeddings.

    :param model_name_or_path: If it is a filepath on disc, it loads the model from that path. If it is not a path, it first tries to download a pre-trained LanguageTransformer model. If that fails, tries to construct a model from Huggingface models repository with that name.
    :param modules: This parameter can be used to create custom LanguageTransformer models from scratch.
    :param device: Device (like 'cuda' / 'cpu') that should be used for computation. If None, checks if a GPU can be used.
    """
    def __init__(self, model_name_or_path: str = None, modules: Iterable[nn.Module] = None, device: str = None):
        if model_name_or_path is not None and model_name_or_path != "":
            logging.info("Load pretrained LanguageTransformer: {}".format(model_name_or_path))
            ...
            if model_path.startswith('http://') or model_path.startswith('https://'):
                ...
                default_cache_path = os.path.join(torch_cache_home, 'language_bert')
                ...
                if not os.path.exists(model_path) or not os.listdir(model_path):
                    ...
                    try:
                        ...
                        if e.response.status_code == 404:
                            logging.warning('LanguageTransformer-Model {} not found. Try to create it from scratch'.format(model_url))
            ...
            if model_path is not None:
                logging.info("Load LanguageTransformer from folder: {}".format(model_path))
    ...
    def start_multi_process_pool(self, target_devices: List[str] = None, encode_batch_size: int = 32):
        ...
        for cuda_id in target_devices:
            p = ctx.Process(target=LanguageTransformer._encode_multi_process_worker, args=(cuda_id, self, input_queue, output_queue, encode_batch_size), daemon=True)
            p.start()
            processes.append(p)
    ...
    def encode_multi_process(self, sentences: List[str], pool: Dict[str, object], is_pretokenized: bool = False, chunk_size=None):
        ...
        :param pool: A pool of workers started with LanguageTransformer.start_multi_process_pool
    ...
    def fit(self,
        ...
        :param evaluator: An evaluator (language_bert.evaluation) evaluates the model performance during training on held-out dev data. It is used to determine the best model that is saved to disc.
        ...
        :param save_iteration: integer: to save model at every n iteration. It will not be used if evalutor is provided
        :param save_epoch: integer: to save model at every n epoch. It will not be used if evalutor is provided
        """
        ...
        logging.info("num_train_objectives %s"%(num_train_objectives))
        ...
        for epoch in trange(epochs, desc="Epoch"):
            ...
            for _ in trange(steps_per_epoch, desc="Iteration", smoothing=0.05):
                ...
                elif(save_iteration!=0 and global_step%save_iteration==0):
                    self.save(output_path + "_cp_" + str(global_step))

            if evaluator:      
                self._eval_during_training(evaluator, output_path, save_best_model, epoch, -1, callback)
            elif(save_epoch!=0 and (epoch%save_epoch==0 or epoch==epochs)):
